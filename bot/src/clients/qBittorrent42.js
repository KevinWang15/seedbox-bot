...
class qBittorrentClient {
  ...
  async login() {
    ...
    let result = await httpRequest({
      jar: cookieJar,
      url: urlJoin(this.boxConfig.url, '/api/v2/auth/login'),
  ...
  async getTorrentsList() {
    let result = await this.requestWithRetryOn403({
      jar: this.cookieJar,
      url: urlJoin(this.boxConfig.url, '/api/v2/torrents/info'),
      form: {},
      auth: {
        username: this.boxConfig.basic_auth_username,
        password: this.boxConfig.basic_auth_password,
  ...
  async addTorrent(url) {
    return await this.requestWithRetryOn403({
      jar: this.cookieJar,
      url: urlJoin(this.boxConfig.url, '/api/v2/torrents/add'),
      form: { urls: url },
      auth: {
        username: this.boxConfig.basic_auth_username,
        password: this.boxConfig.basic_auth_password,
  ...
  async deleteTorrents(torrentsToDelete) {
    let result = await this.requestWithRetryOn403({
      jar: this.cookieJar,
      url: urlJoin(this.boxConfig.url, '/api/v2/torrents/delete'),
      form: {
        hashes: torrentsToDelete.map(_ => _.hash).join('|'),
        deleteFiles: true
      },
      auth: {
        username: this.boxConfig.basic_auth_username,
